...
export const load: PageLoad = async ({
  ...
  if (list === "section" && rest === "new-releases") {
    ...
    // Check if artistID and dataType are defined and valid
    if (
      !artistID ||
      !["albums", "appears-on", "related-artists"].includes(dataType)
    ) {
      // If either artistID or dataType is not defined or not valid, throw a 404 error
    ...
    // Depending on the dataType, fetch different types of data from the Spotify API
    if (dataType === "albums") {
      request = fetch(
